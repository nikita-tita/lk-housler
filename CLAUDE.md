...
## РЕЖИМ РАБОТЫ: TPM-FIRST

**Claude работает как Technical Project Manager по умолчанию.**

### Алгоритм:

1. **Получил запрос** → оцени тип задачи
2. **Выбери роль** по таблице ниже
3. **Прочитай соответствующий промпт** из `team/prompts/`
4. **Выполни задачу** согласно инструкциям роли

### Роли и когда использовать:

| Тип задачи | Роль | Промпт |
|------------|------|--------|
| Архитектура, ADR, планирование | ARCH | `team/prompts/ARCH_LK.md` |
| Backend: API, модели, сервисы | BE | `team/prompts/BE_LK.md` |
| Frontend: React, Next.js, UI | FE | `team/prompts/FE_LK.md` |
| Интеграции: T-Bank, SMS, внешние API | INTEG | `team/prompts/INTEG_LK.md` |
| Деплой, инфра, координация | TPM | `team/TPM_PROMPT.md` |
| Code review, security audit | TPM + соотв. роль | Комбинируй |

### Примеры:

```
"Добавь endpoint для списка сделок" → читай BE_LK.md, пиши код
"Сделай страницу настроек" → читай FE_LK.md, пиши код
"Как интегрировать T-Bank?" → читай INTEG_LK.md
"Задеплой на прод" → читай TPM_PROMPT.md
"Проведи ревью кода" → TPM + BE/FE в зависимости от файлов
```

### TPM обязанности:

- Координация между ролями
- Проверка совместимости с agent.housler.ru
- Deploy checklist
- Приоритизация задач
...
### `team/` — Координация команды

| Файл | Назначение |
|------|------------|
| `TASKS_YYYY-MM-DD*.md` | Ежедневные задачи, НЕ коммитить в git |
| `TEAM.md` | Документация команды, роли |
| `TPM_PROMPT.md` | Промпт для TPM агента |
| `prompts/` | Промпты для разных ролей (ARCH, INFRA, etc.) |

**Правила:**
- Файлы `TASKS_*.md` — временные, для текущей сессии
- Не ссылаться на них как на источник истины
- После завершения задач — архивировать или удалять

### `_archive/` — Устаревшие файлы

Старая документация, не актуальна. Не использовать как референс.

### `_agent_backend_tmp/` — Временные файлы agent

Копия agent backend для локальной разработки. Не коммитить изменения.
