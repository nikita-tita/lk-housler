...
export interface OfferCardProps {
  ...
  usePlanImage?: boolean; // Показывать планировку вместо фото ЖК
}

export function OfferCard({ offer, highlighted, compact, linkPrefix, usePlanImage }: OfferCardProps) {
  const offerUrl = linkPrefix ? `${linkPrefix}/offers/${offer.id}` : `/offers/${offer.id}`;
  // Если usePlanImage, предпочитаем планировку, иначе фото ЖК
  const displayImage = usePlanImage
      ...
      <div className="aspect-[4/3] bg-[var(--color-bg-gray)] relative overflow-hidden">
        {displayImage ? (
          <img
            ...
            className={`w-full h-full group-hover:scale-105 transition-transform duration-300 ${usePlanImage ? 'object-contain p-2' : 'object-cover'}`}
          />
        ) : (
          <div className="w-full h-full flex items-center justify-center text-[var(--color-text-light)]">
