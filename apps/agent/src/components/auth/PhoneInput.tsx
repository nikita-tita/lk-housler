...
function getCursorPosition(digits: string, cursorInDigits: number): number {
  // Маппинг: позиция в цифрах -> позиция в форматированной строке
  // +7 (999) 123-45-67
  // Индексы цифр: 7=1, 9=4,5,6, 1=9,10,11, 4=13,14, 6=16,17
  const positions = [2, 4, 5, 6, 9, 10, 11, 13, 14, 16, 17];

  if (cursorInDigits <= 0) return 2; // После +7
  if (cursorInDigits >= digits.length) {
    return formatPhone(digits).length;
  }

  // Позиция после cursorInDigits-й цифры (0-based, но первая цифра это 7)
...
export const PhoneInput = forwardRef<HTMLInputElement, PhoneInputProps>(
  ({ value, onChange, error, className = '', ...props }, ref) => {
    ...
    // Устанавливаем курсор после рендера
    useEffect(() => {
    ...
    const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      // Считаем сколько цифр до курсора в новом значении
    ...
    // Комбинируем внешний ref и внутренний
