...
async def list_templates(
    ...
    templates, total = await service.list_templates(
        code=code, status=status, active_only=active_only, limit=limit, offset=offset
    )
    return TemplateListResponse(items=[TemplateListItem.model_validate(t) for t in templates], total=total)


@router.get("/{template_id}", response_model=TemplateResponse)
async def get_template(
    template_id: UUID, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)
):
    """Get template by ID"""
    service = TemplateService(db)
...
async def create_template(
    data: TemplateCreate, current_user: User = Depends(require_admin), db: AsyncSession = Depends(get_db)
):
...
async def delete_template(
    template_id: UUID, current_user: User = Depends(require_admin), db: AsyncSession = Depends(get_db)
):
...
async def validate_template(
    ...
    result = service.validate_template(template.template_body, template.placeholders_schema)
...
async def preview_template(
    ...
    result = service.preview_template(template.template_body, data.test_data)
...
async def seed_templates(current_user: User = Depends(require_admin), db: AsyncSession = Depends(get_db)):
    """Seed database with hardcoded templates (Admin only)"""
    service = TemplateService(db)
    created = await service.seed_hardcoded_templates()
    await db.commit()

    return {"message": f"Created {len(created)} templates", "templates": [t.code for t in created]}
